...
entity copy1 is --0x7f5f30004250
    generic (
        ram_width : natural;
        ram_depth : natural
    );
...
architecture copy1_arch of copy1 is
    ...
    component inputnode port (
        in_R0x7f5f30004250 : out std_logic_vector(ram_width - 1 downto 0);
        out_R0x7f5f30004250 : in std_logic_vector(ram_width - 1 downto 0)
    ); end component;

    component outputnode port (
        --out_channel_0_real_vect : in std_logic_vector(ram_width - 1 downto 0);
        out_ROUTPUT_0 : in std_logic_vector(ram_width - 1 downto 0);         
        in_ROUTPUT_0 : out std_logic_vector(ram_width - 1 downto 0)
    ); end component;

    --component interior port (
    --    out_channel_0_real_vect : in std_logic_vector(ram_width - 1 downto 0);
    --    in_channel_0_real_vect : out std_logic_vector(ram_width - 1 downto 0)
    --); end component;

    signal inputnode, outputnode : std_logic; 

    -- buffer stuff begins here
    ...
    -- buffer stuff ends here

    data_input : process(clk)
    begin     
      if rst = '1' then
        -- copying data into input inputnode (from first memory)
        out_R0x7f5f30004250 <= in_R0x7f5f30004250;
        -- copying input to buffer (channel_0x7f5f30004250)
        in_data <= in_R0x7f5f30004250;
      end if;
    end process;
    ...
    begin     
        if rising_edge(clk) then
            if rst = '1' then
            -- copying buffer into output (channel_OUTPUT_0)
            in_ROUTPUT_0 <= out_data

            -- copying data out of output (for second memory)
            out_ROUTPUT_0 <= in_ROUTPUT_0;
            end if;
        end if;
    end process;
